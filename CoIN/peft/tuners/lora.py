...
def mark_only_current_lora_as_trainable(model: nn.Module, bias: str = "none", current_step: int = 0) -> None:
    for n, p in model.named_parameters():
        if "lora_" not in n:
            p.requires_grad = False
        elif "lora_router" not in n and int(n.split(".")[-2]) in range(current_step):
            p.requires_grad = False
            print(str(n)+" set False requires_grad")
        elif "lora_router" in n and "current_key" in n:
            p.requires_grad = True
            print(str(n)+" set True requires_grad")
        elif "lora_router" in n and "default" in n:
            p.requires_grad = False
            print(str(n)+" set False requires_grad")
...
def mark_current_lora_wo_router_as_trainable(model: nn.Module, bias: str = "none", current_step: int = 0) -> None:
    for n, p in model.named_parameters():
        if "lora_" not in n:
            p.requires_grad = False
        elif "lora_router" not in n and int(n.split(".")[-2]) in range(current_step):
        ...
        if "prompt_key_share" in n:
            p.requires_grad = True
            print(str(n)+" set True requires_grad")
        elif "old_pro_key_share" in n:
            p.requires_grad = False
            print(str(n)+" set False requires_grad")
        elif "old_tra_input_share" in n:
            p.requires_grad = False
            print(str(n)+" set False requires_grad")
        elif "trans_input_share" in n:
            p.requires_grad = True
            print(str(n)+" set True requires_grad")
