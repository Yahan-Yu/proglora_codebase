...
class ModelArguments:
    ...
    lora_method: Optional[str] = field(default=None)
    task_id: Optional[int] = field(default=8)
    trans_hidden_dim: Optional[int] = field(default=100)
    # lora_add_method: Optional[str] = field(default="full")
    # target_layers: Optional[str] = field(default="0-31")
...
def parse_range_string(range_str: str) -> List[int]:
    """
    将包含数字和范围（用逗号和连字符分隔）的字符串解析为整数列表。
    所有数字必须在 [0, 31] 的范围内。

    示例:
    '1-32' -> [1, 2, ..., 32]
    '1,2,3,6-9,14' -> [1, 2, 3, 6, 7, 8, 9, 14]
    """
    ...
    elements = [e.strip() for e in range_str.split(',') if e.strip()]
        ...
        if '-' in element:
            try:
                ...
                if start < MIN_VAL or end > MAX_VAL or start > end:
                    raise ValueError(f"number value is not in [0, 31]: {element}")
                
                result.update(range(start, end + 1))
                
            except ValueError as e:
                raise ValueError(f"invalid number in lora add layers: {element}. details: {e}")
            ...
            try:
                ...
                if num < MIN_VAL or num > MAX_VAL:
                    raise ValueError(f"number value is not in [0, 31]: {num}")
    ...
    print("Lora is added in layer: "+ str(sorted(list(result))))
...
def train():
    ...
    if training_args.lora_enable:
        kwargs = { 
            ...
            "lora_method": model_args.lora_method,
            "task_id": model_args.task_id,
            "trans_hidden_dim": model_args.trans_hidden_dim,
            # "lora_add_method": model_args.lora_add_method,
            # "target_layers": parse_range_string(model_args.target_layers),
            }
